...
4. **Client and relay share one origin** at <https://sneakbit.curzel.it> — the VPS serves the static client and brokers co-op (nginx serves the bundle at `/`, proxies the relay on `/ws` + the JSON endpoints).
