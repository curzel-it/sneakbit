...
use rendering::{textures::load_tile_map_textures, ui::{get_rendering_config, get_rendering_config_mut, CameraDirection}, window::{handle_window_updates, load_last_fullscreen_settings, start_rl}, worlds::render_frame};
...
fn main() {
    ...
    loop {
        ...
        if let Some(new_camera_direction) = CameraDirection::from_player(&current_main_player_direction()) {
            get_rendering_config_mut().direction = new_camera_direction;
        }
