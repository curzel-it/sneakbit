...
test("small + blueberry → strawberry (lower tier absorbs into higher)", () => {
  ...
  assert.equal(world.entities.length, 1);
  assert.equal(world.entities[0].id, 2);
  assert.equal(world.entities[0].species_id, 4006);
});
