...
export async function startCoopSession({
  ...
  const inviteCode = await waitFor(host, `
    (async () => {
      const o = await import('./js/onlineBootstrap.js?v=20260531a');
  ...
  if (entry === "deeplink") {
    ...
    await evalExpr(guest, `
      (async () => {
        const sr = await import('./js/switchRole.js?v=20260531a');
  ...
  await waitFor(guest, `
    (async () => {
      ...
      const p = await import('./js/predictedSelf.js?v=20260531a');
      const o = await import('./js/onlineBootstrap.js?v=20260531a');
      window.__sb = { m, p, o };
