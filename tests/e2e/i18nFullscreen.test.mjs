...
test("localization loads English by default and Italian when selected; fullscreen button present", async (t) => {
  ...
  const enWeapons = await evalExpr(s, `import('./js/strings.js?v=20260530e').then(m => m.tr('weapons_selection.title'))`);
  ...
  await navigate(s, `${servers.appUrl}/index.html`);
  await waitFor(s, "!!window.__menuNav");
  const itWeapons = await evalExpr(s, `import('./js/strings.js?v=20260530e').then(m => m.tr('weapons_selection.title'))`);
  ...
  const fallback = await evalExpr(s, `import('./js/strings.js?v=20260530e').then(m => m.tr('ok'))`);
